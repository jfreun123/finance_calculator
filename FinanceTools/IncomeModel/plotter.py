...
class Plotter:
    ...
    def plot_all(self, number_of_simulations):
        ...
        ax2.axvline(mean_val, color='darkblue', linestyle='--', linewidth=2, label=f"Mean = {mean_val:,.0f}")
        ax2.axvline(p50, color='green', linestyle='--', linewidth=2, label=f"Median = {p50:,.0f}")
        ax2.axvline(p5, color='red', linestyle=':', linewidth=1.5, label=f"5th %ile = {p5:,.0f}")
        ax2.axvline(p95, color='red', linestyle=':', linewidth=1.5, label=f"95th %ile = {p95:,.0f}")
