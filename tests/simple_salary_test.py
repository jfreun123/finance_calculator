...
bloomberg = SimpleSalary(
    post_tax_post_roth_base_only_monthly_income=3950 * 2,
    monthly_rent=2293,
    monthly_expense=3000,
    post_tax_yearly_bonus=0.5 * 31_000,
    yearly_roth=22_500,
)
...
def test_bloomberg():
    ...
    assert (
        bloomberg.recommended_yearly_savings()
        == 15500 + 22500 + (3950 * 2 - (2293 + 3000)) * 12
    )
...
def test_inflation_bloomberg():
    inflation_bloomberg = InflationAdjustedSalary(
        simple_salary=bloomberg, inflation_rate=0.03
    )
    ...
    assert inflation_bloomberg.recommended_yearly_savings(
        years=10
    ) == curr_expected_savings * (1.03**10)
